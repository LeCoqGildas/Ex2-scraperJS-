...
*/
function scraper(){
	var page = document.getElementById("hashtag").value;
	//this console take the current date :) and add color(it's good on windows!)
	//green => date
	//red => socket, and loading
	//yellow => information about page
	...
	//schema, we take the urls and the current date
	var linkUrlSchema = new mongoose.Schema({
	  linkUrl : String,
	  //date : { type : Date, default : Date.now }//I not use this, beacause I have problemes in mongoDB, it take full of duplicates url
	});
	//model used
	var urlModel = mongoose.model('urls', linkUrlSchema);
	//create an object, 
	var theLinkUrl;

	//how to create new data
	//theLinkUrl = new urlModel({ linkUrl : url });

	//how to add data in the bdd
	...
	*/
	function extract_links(page_url, html_str){
		...
		(html_str.match(EXTRACT_URL_REG) || []).forEach(function(url){
			...
			console.log(url);
			//if the queue contains the url, we not add this
			if(queue.queue.indexOf(url) < 0){
				// Add the url to the queue
				queue.push(url);
				document.getElementById("newUrl").innerHTML += '<tr><td width="100%">'+url+'</td><td><input type="button" id='+url+' name="infos" value="INFOS URL" onClick="get_page_info(\''+url+'\');"></td></tr>';
				em.emit('url', page_url, html_str, url);
			}else{
				console.log("take an url already crawled");
			}
		});
	...
	function handle_new_url(from_page_url, from_page_str, url){
		...
		//Search in the bdd if the url exsit and add this if it not exist
		var query = urlModel.find("urls");
		query.where('linkUrl').limit(1);
		
		query.exec(function (err, obj) {
			if (err) {throw err;}	
			if(!(obj[0])){
				theLinkUrl = new urlModel({ linkUrl : url });
				theLinkUrl.save(function() {
					console.red('One Link is add to the BDD !');
					get_page(url);
				});
			}else{console.red("This Url il already in the BDD");};
		});
	...
	app.listen(PORT);
	console.red('Web UI Listening on port '+PORT);
